...
const getCardConfiguration = () => ({
  title: "CO₂ Flow Rate",
  unit: "Kg/Hr",
  ranges: { min: 1.5, max: 3.5 },
  normalRange: { min: 2.0, max: 2.8 },
...
const renderIcon = () => (
  <div className="relative">
    ...
    <div className="relative p-3 bg-gradient-to-br from-blue-500/30 to-blue-600/20 rounded-xl border border-blue-400/30 backdrop-blur-sm">
      <div className="w-6 h-6 flex items-center justify-center">
        <span className="text-blue-300 font-bold text-xs">CO2</span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative h-2 bg-white/10 rounded-full overflow-hidden">
          ...
          <div 
            ...
            style={{ left: `${(config.normalRange.min / config.ranges.max) * 100}%` }}
          ></div>
          <div 
            className="absolute top-0 w-0.5 h-full bg-white/60"
            style={{ left: `${(config.normalRange.max / config.ranges.max) * 100}%` }}
          ></div>
        </div>
      </div>
...
// Main Component
export const CO2FlowCard: React.FC<CO2FlowCardProps> = ({ value }) => {
  const config = getCardConfiguration()
